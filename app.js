(() => {
  ...
  function show(n){current=Math.max(1,Math.min(totalSlides,n));$('#slide').src=`slide${pad(current)}.png`;$('#count').textContent=`${current} / ${totalSlides}`;$('#context').textContent=title();const isTimed=(current>=6&&current<=30)||(current>=32&&current<=46);$('#timer-box').classList.toggle('closed',!isTimed);if(isTimed)resetTimer((current>=32&&current<=46)?10:45);if(current===91)$('#scoreboard').classList.remove('closed')}
