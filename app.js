(() => {
  const totalSlides=95, people=['태화','명우','범서','채린','성민','가현','지은','지안'];
  ...
  function show(n){current=Math.max(1,Math.min(totalSlides,n));$('#slide').src=`https://raw.githubusercontent.com/syhan20060126-code/-MT/main/slide${pad(current)}.png`;$('#count').textContent=`${current} / ${totalSlides}`;$('#context').textContent=title();const isTimed=(current>=6&&current<=30)||(current>=32&&current<=46);$('#timer-box').classList.toggle('closed',!isTimed);if(isTimed)resetTimer((current>=32&&current<=46)?10:45);if(current===91)$('#scoreboard').classList.remove('closed')}
  ...
  $('#score-edit').onclick=()=>{ $('#inputs').innerHTML=people.map(p=>`<label class="input-row"><span>${p}</span><input type="number" inputmode="numeric" value="${scores[p]}"></label>`).join('');$('#mission-award').style.display=current===3?'inline-block':'none';$('#score-dialog').showModal()};
